...
class moveseq {
public:
   moveseq(int sz=0) : _size(sz) {}
   void push_back(int v) { moves[_size++] = v ; }
   void pop_back() { _size-- ; }
   int size(void) const { return _size ; }
   int &operator[](int v) { return moves[v] ; }
   const int &operator[](int v) const { return moves[v] ; }
   int *begin() { return &moves[0] ; }
   int *end() { return &moves[_size] ; }
private:
   int moves[40] ;
   int _size ;
} ;
...
@(cubepos_test.cpp@>=
const unsigned int MAXELEMENTS = 3000 ;

...
@<Utility methods@> @;
@<Method bodies@> @;
void phase2prune::init(int maxdep, int suppress_writing) {
...
@<Method decl...@>=
static int quick_pop(permcube pc, int togo, int v, int canon, int mm) ;
...
@<Method bodies@>=
int phase2prune::quick_pop(permcube pc, int togo, int v, int canon, int mm) {
   if (togo == 0) {
      int cc = corner_coordinate(pc) ;
      corner_reduce &cr = corner_reduction[cc] ;
      int ec = edge_coordinate(pc) ;
      int off = cr.c * FACT8 + edgeud_remap[cr.m][ec] ;
      int bits = 4*(off&7) ;
      off >>= 3 ;
      if (((mem[off]>>bits) & 0xf) == 0xf) {
         int rv = 1 ;
         mem[off] -= (0xf - v) << bits ;
         if (cr.minbits & (cr.minbits - 1)) {
            for (int m=cr.m + 1; (1<<m) <= cr.minbits; m++)
               if ((cr.minbits >> m) & 1) {
                  off = cr.c * FACT8 + edgeud_remap[m][ec] ;
                  bits = 4*(off&7) ;
                  off >>= 3 ;
                  if (((mem[off]>>bits) & 0xf) == 0xf) {
                     mem[off] -= (0xf - v) << bits ;
   ...
   int mvmask = cubepos::cs_mask(canon) & mm & 0227227 ;
   permcube pc2 ;
   togo-- ;
   int r = 0 ;
   for (int mv=0; mv<NMOVES; mv++) {
      ...
      pc2 = pc ;
      pc2.move(mv) ;
      r += quick_pop(pc2, togo, v, cubepos::next_cs(canon, mv), -1) ;
   }
   return r ;
}
void phase2prune::gen_table(int maxdep) {
   printf("Gen phase2 so far %g\n", duration()) ;
   fflush(stdout) ;
   ...
   for (int d=0; d<maxdep; d++) {
      ...
      if (d <= 8) {
         pc = identity_pc ;
         seen += quick_pop(pc, d+1, newval, CANONSEQSTART, 0x13) ;
      } else {
#endif
         for (int c8_4=0; c8_4<C8_4; c8_4++)
            for (int ctp=0; ctp<FACT4; ctp++)
   ...
   /* correct high entries */
   unsigned int setval = maxdep ;
   setval |= setval << 4 ;
   setval |= setval << 8 ;
   setval |= setval << 16 ;
   for (int i=0; 4*i<memsize; i++) {
      unsigned int v = mem[i] ;
      if (v == 0xffffffff)
         mem[i] = setval ;
      else {
         v &= v>>1 ;
         mem[i] -= (v & 0x11111111 & (v>>2)) * (15 - maxdep) ;
      }
   }
   printf(" done %g\n", duration()) ;
   fflush(stdout) ;
   ...
   if (backwards) {
      ...
      for (int etp=0; etp<FACT4; etp++)
       if (mem[off + (at >> 3)] == 0xffffffff &&
           mem[off + (at >> 3) + 1] == 0xffffffff &&
           mem[off + (at >> 3) + 2] == 0xffffffff) {
          at += 24 ;
       } else
         for (int ebpo=0; ebpo<FACT4; ebpo += 8) {
            if (mem[off + (at >> 3)] != 0xffffffff) {
